...
function lastInWorld($db,$imageIdList){
  ...
  $last_photos_comments_data = $db->prepare("SELECT c.text_comment, c.id_photo, u.avatar, u.name FROM users u INNER JOIN comments c ON c.id_user = u.id_user INNER JOIN photo p ON c.id_photo = p.id_photo WHERE p.id_photo IN ($photos_id_list) ORDER BY c.id_comment");
