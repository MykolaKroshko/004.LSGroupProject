...
function lastInWorld($db,$imageIdList){
  $last_photos_data = $db->prepare("SELECT p.id_photo, p.id_album, p.photo, p.description, p.source, u.name, u.avatar FROM users u INNER JOIN albums a ON a.id_user = u.id_user INNER JOIN photo p ON p.id_album = a.id_album WHERE p.id_photo NOT IN ($imageIdList) ORDER BY p.id_photo DESC limit 6");
